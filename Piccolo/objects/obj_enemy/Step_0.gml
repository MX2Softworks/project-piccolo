...
if((current_node == "" || current_node == undefined ) && ds_list_size(path) > 0){
...
//once we reach the current node, set the new current node to next node in path, and prev to old current.
if(!is_undefined(current_node) && current_node != "" 
...
if(is_wall_jump && !is_undefined(next_node) && next_node[1] < current_node[1] && next_node[0] == current_node[0]
   ...
   && wall_jump){
	if(is_wall_jump == 1){
		...
		file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
		file_text_write_string(file, "wall_jump == 1 && next_node y > curr_node y && next_node x = curr_node x && next_node wall_jump && !on_ground { direction_horizontal = -1; }");
		file_text_writeln(file);
		file_text_close(file);
	}
	else{
		direction_horizontal = +1;
		file_text_write_string(file, "wall_jump == 2 && next_node y > curr_node y && next_node x = curr_node x && next_node wall_jump && !on_ground { direction_horizontal = +1; }");
	}
}
else if((is_wall_jump == 2 && wait_for_wall_jump) || current_node[0]/*+current_hspd*/ > curr_chunk_x){
			direction_horizontal =  1;
			file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
			file_text_write_string(file, "wall_jump == 2 && wait_for_wall_jump or go right { direction_horizontal = +1; }");
			file_text_writeln(file);
			file_text_close(file);
		}
			...
			direction_horizontal = -1;
			file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
			file_text_write_string(file, "wall_jump == 1 && wait_for_wall_jump or go left { direction_horizontal = -1; }");
			file_text_writeln(file);
			file_text_close(file);
		}
else if(curr_chunk_x == current_node[0]){ 
	direction_horizontal =  0;
	file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
	file_text_write_string(file, "curr_chunk_x == current_node { direction_horizontal = 0; }");
	file_text_writeln(file);
	file_text_close(file);
}
...
if(current_node[1] < curr_chunk_y)		{
	...
	file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
	file_text_write_string(file, "go up { up = 1; up_released = 0; up_held = 1; }");
	file_text_writeln(file);
	file_text_close(file);
}
else									{
	...
	file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
	file_text_write_string(file, "fall { up = 1; up_released = 0; up_held = 1; }");
	file_text_writeln(file);
	file_text_close(file);
}
...
//if the next node is also midair, keep jump pressed.
else*/ if(!is_undefined(next_node) && next_node[1] == current_node[1] 
     && !scr_pathfinder_is_on_ground(pathfinder, current_node[0], current_node[1])){
	up = 1; up_released = 0; up_held = 1;
	file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
	file_text_write_string(file, "if next node is also midair, keep jump pressed { up = 1; up_released = 0; up_held = 1; }");
	file_text_writeln(file);
	file_text_close(file);
}

//hold left or right in the direction of the next node, if the current is in midair.
if(!is_undefined(next_node) && scr_pathfinder_is_on_ground(pathfinder, next_node[0], next_node[1])
   ...
   && curr_chunk_y <= current_node[1]){
	...
	if(next_node[0] > current_node[0]){
		...
		direction_horizontal = 1;
		file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
		file_text_write_string(file, "then hold right if the current node is in midair { direction_horizontal = 1; }");
		file_text_writeln(file);
		file_text_close(file);
	}
	if(next_node[0] < current_node[0]){
		direction_horizontal = -1;
		file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
		file_text_write_string(file, "then hold left if the current node is in midair { direction_horizontal = -1; }");
		file_text_writeln(file);
		file_text_close(file);
	}
...
if(!is_undefined(current_node) && current_node != "" && current_node[1] < curr_chunk_y && current_node[0] == curr_chunk_x
  && place_meeting(x, y-30, obj_solid)){
	...
	|| !place_meeting(x+(sprite_width*2), y-30, obj_solid)){
		direction_horizontal = +1;
		file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
		file_text_write_string(file, "path upward causes collision and right is open { direction_horizontal = +1; }");
		file_text_writeln(file);
		file_text_close(file);
	}
	if(!place_meeting(x-(sprite_width/2), y-30, obj_solid) 
	  ...
	  || !place_meeting(x-(sprite_width*2), y-30, obj_solid)){
		direction_horizontal = -1;
		file = file_text_open_append("C:\Users\Deadb\Desktop\ai_debug_state.txt");
		file_text_write_string(file, "path upward causes collision and left is open { direction_horizontal = -1; }");
		file_text_writeln(file);
		file_text_close(file);
	}
}
...
//Handes wall jump cases where there is an extra node marking the peak of the jump. 
if(prev_node != "" && !is_undefined(prev_node) && current_node != "" && !is_undefined(current_node) && next_node != "" && !is_undefined(next_node)){
	if(scr_pathfinder_is_wall_jump(pathfinder, prev_node[0], prev_node[1]) == 1 
	&& scr_pathfinder_is_on_ground(pathfinder, next_node[0], next_node[1]) && direction_to_next[0] < 0){
		direction_horizontal = -1; 
	}
	else if(scr_pathfinder_is_wall_jump(pathfinder, prev_node[0], prev_node[1]) == 2
	&& scr_pathfinder_is_on_ground(pathfinder, next_node[0], next_node[1]) && direction_to_next[0] > 0){
		direction_horizontal = +1;
	}
}
